...
pub fn build() -> Result<EspIdfBuildOutput> {
    ...
    // Unlike the standalone cargo driver, this driver has no exported IDF
    // environment to search for the GCC linker, so ask the compiler that the
    // ESP-IDF CMake build already passed us. Needed for the picolibc include
    // detection when CONFIG_LIBC_PICOLIBC=y (ESP-IDF 6 default).
